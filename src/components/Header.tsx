    ...
    <header className="fixed top-0 w-full z-50 backdrop-blur-md bg-transparent">
      <div className="max-w-7xl mx-auto p-4 flex items-center justify-between">
        ...
        <nav className="flex gap-6">
          <Link to="/" className="text-white font-medium hover:text-indigo-400">Home</Link>
          <Link to="#" className="text-white font-medium hover:text-indigo-400">Eventos</Link>
          <Link to="/" className="text-white font-medium hover:text-indigo-400">Dudas</Link>
        </nav>
        <div className="flex gap-4">
          <button className="border border-white rounded-full px-4 py-2 text-white hover:shadow-lg transition">Iniciar Sesión</button>
          <button 
      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-30 z-50 pt-80">
          <div className="bg-white p-8 rounded-xl shadow-xl w-full max-w-md relative">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <label className="text-left text-sm">Nombre Completo:
